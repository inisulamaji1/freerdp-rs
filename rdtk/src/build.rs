...
fn main() {
    let library = pkg_config::probe_library("rdtk0")
        .expect("pkg-config could not find rdtk0. Try setting PKG_CONFIG_PATH if not installed in system directories");
    ...
    writeln!(&mut header, "#ifndef RDTK_0_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "#define RDTK_0_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
    ...
    writeln!(&mut header, "").expect(ferr);
    writeln!(&mut header, "#endif /* RDTK_0_WRAPPER_HEADER */").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
