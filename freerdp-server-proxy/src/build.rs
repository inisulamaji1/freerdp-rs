...
fn main() {
    let library = pkg_config::probe_library("freerdp-server-proxy3")
        .expect("pkg-config could not find freerdp-server-proxy3. Try setting PKG_CONFIG_PATH if not installed in system directories");
    ...
    writeln!(&mut header, "#ifndef FREERDP_SERVER_PROXY_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "#define FREERDP_SERVER_PROXY_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
    ...
    for path in &library.include_paths {
        ...
        if !spath.contains("freerdp-server-proxy3") {
            continue;
        }
    ...
    writeln!(&mut header, "").expect(ferr);
    writeln!(&mut header, "#endif /* FREERDP_SERVER_PROXY_3_WRAPPER_HEADER */").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
